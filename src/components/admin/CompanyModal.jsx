...
import { crearEmpresa, actualizarEmpresa } from '../../services/empresasService';
...
export default function CompanyModal({ empresa, rubros, onClose, onGuardado }) {
  ...
  const [form, setForm] = useState({
    nombre: '', codigo: '', email: '', telefono: '', rubro: '',
    direccion: '', nombre_contacto: '', porcentaje_comision: '', sitio_web: '',
  });
  ...
  const validar = () => {
    ...
    if (!form.nombre_contacto.trim()) e.nombre_contacto = 'Nombre de contacto requerido';
    if (!validarComision(form.porcentaje_comision)) e.porcentaje_comision = 'Debe ser un número entre 0 y 100';
    return e;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (uploadingLogo) { setErrorGeneral('Espera a que termine de subir la imagen.'); return; }
    setLoading(true);
    setErrorGeneral('');
    try {
      const datos = {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-2xl border border-cream w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl animate-slide-up">
        ...
        <form id="company-form" onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          {/* Logo uploader */}
          <div>
            <label className="text-sm font-medium text-navy block mb-1.5">Logo de la empresa</label>
            <div className="flex items-center gap-4">
              ...
              <div className="w-16 h-16 rounded-xl border-2 border-dashed border-cream bg-cream-bg flex items-center justify-center flex-shrink-0 overflow-hidden">
                {uploadingLogo ? (
                  <svg className="animate-spin w-5 h-5 text-teal" viewBox="0 0 24 24" fill="none">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {uploadingLogo ? 'Subiendo...' : logoPreview ? 'Cambiar imagen' : 'Subir imagen'}
                </button>
            ...
            <input
              ...
              className="hidden"
            />
            <p className="text-xs text-navy/40 mt-1.5">PNG, JPG o SVG. Se sube automáticamente al seleccionar.</p>
          </div>

          <Input label="Nombre de la empresa" required value={form.nombre} onChange={handleChange('nombre')} error={errores.nombre} placeholder="Ej: Restaurante San Juan" />

          <div className="grid grid-cols-2 gap-3">
            <Input label="Código" required value={form.codigo} onChange={handleChange('codigo')} error={errores.codigo} placeholder="ABC123" hint="3 letras + 3 dígitos" />
            <Input label="% Comisión" required type="number" min="0" max="100" value={form.porcentaje_comision} onChange={handleChange('porcentaje_comision')} error={errores.porcentaje_comision} placeholder="Ej: 5" />
          </div>

          <div>
            <label className="text-sm font-medium text-navy block mb-1.5">Rubro <span className="text-coral ml-0.5">*</span></label>
            <select value={form.rubro} onChange={handleChange('rubro')}
            ...
            {errores.rubro && <p className="text-sm text-coral mt-1">{errores.rubro}</p>}
          </div>

          <Input label="Nombre del contacto" required value={form.nombre_contacto} onChange={handleChange('nombre_contacto')} error={errores.nombre_contacto} placeholder="Ej: Juan García" />

          <div className="grid grid-cols-2 gap-3">
            <Input label="Correo" required type="email" value={form.email} onChange={handleChange('email')} error={errores.email} placeholder="contacto@empresa.com" />
            <Input label="Teléfono" required value={form.telefono} onChange={handleChange('telefono')} error={errores.telefono} placeholder="12345678" />
          </div>
