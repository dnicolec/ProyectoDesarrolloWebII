...
const OfferDetailPage = ({ user }) => {
  ...
  const handleAgregarCupon = async () => {
    ...
    try {
      // Una sola llamada: crea el cupón y lo asigna en una transacción atómica
  ...
  const descuentoTexto = offer.tipo === 'porcentaje'
    ? `${offer.descuento}%`
    : `$${offer.descuento}`;
  ...
  const botonDeshabilitado = processing || disponibles <= 0 || diasRestantes <= 0 || yaLoTiene;
    ...
    <div className="container-app py-8">
      <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
        ...
        <div className="md:col-span-3 space-y-6">
          <div className={`relative rounded-2xl overflow-hidden h-72 bg-gradient-to-br ${gradient}`}>
          ...
          <div>
            <h2 className="font-bold text-navy mb-2">Información de la Empresa</h2>
            <div className="text-navy/60 space-y-1">
              <p><strong>{offer.empresa?.nombre}</strong></p>
        ...
        <div className="md:col-span-2">
          <div className="sticky top-24 bg-cream-bg rounded-2xl border border-cream p-6 space-y-5">
            ...
            <h1 className="font-serif text-2xl font-extrabold text-navy">{offer.titulo}</h1>

            <div className="text-center space-y-2">
              <div className="text-3xl font-black text-teal">{descuentoTexto}</div>
            ...
            <div className="bg-navy/5 rounded-lg p-4 text-sm text-navy/70">
              ...
              <p>{Math.max(0, disponibles)} de {offer.cantidadCupones} cupones disponibles</p>
            </div>
