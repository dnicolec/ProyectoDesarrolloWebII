...
export const canjearCupon = async (cuponId) => {
    try {
        ...
        if (cuponData.usuario_id !== user.uid) throw new Error('Este cupón no te pertenece');
        if (cuponData.estado === 'canjeado') throw new Error('Este cupón ya fue canjeado');
        if (cuponData.estado !== 'asignado') throw new Error('Este cupón no puede ser canjeado');
    ...
    try {
        ...
        querySnapshot.docs.map(async (cuponDoc) => {
            ...
            if (ofertaData.empresa_id) {
                const empresaSnap = await getDoc(doc(db, "empresas", ofertaData.empresa_id));
...
export const generarCupones = async () => {
    console.warn('generarCupones() está deprecada. Usa solicitarCupon() en su lugar.');
    return { success: false, mensaje: 'Función deprecada' };
};

/** @deprecated Usar solicitarCupon() en su lugar */
export const asignarCuponPorOferta = async (ofertaId) => {
    console.warn('asignarCuponPorOferta() está deprecada. Usa solicitarCupon() en su lugar.');
